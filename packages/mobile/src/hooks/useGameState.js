...
function createDefaultState() {
  return {
    ...
    catchResult: null,
    dungeonRun: null,
    collection: buildStartingCollection(),
    squadIds: [],
    encounterHistory: {},
  };
}
...
async function loadInitialState() {
  ...
  try {
    const savedCollection = await AsyncStorage.getItem('8gents_collection');
  ...
  try {
    const savedSquadIds = await AsyncStorage.getItem('8gents_squadIds');
  ...
  try {
    const savedEncounterHistory = await AsyncStorage.getItem('8gents_encounterHistory');
...
async function persist(collection, squadIds, encounterHistory) {
  try {
    await AsyncStorage.multiSet([
      ['8gents_collection', JSON.stringify(collection)],
      ['8gents_squadIds', JSON.stringify(squadIds)],
      ['8gents_encounterHistory', JSON.stringify(encounterHistory)],
    ]);
  } catch (e) {
...
export const useGameState = create((set, get) => ({
  ...
  // Initialize persisted state on app start
  _initialized: false,
  ...
  handleDungeonContinue: () => {
    const { dungeonRun, result, currentEncounter } = get();
