#!/usr/bin/env node
// ─── PROTOTYPE 4: buff the STARTER creatures (the onboarding fix #2) ─────────────
// The Anchor's dead weight is its two Commons: Fizzpop (Reactor 220/30) and Stoneward
// (Bulwark 320/18). Rather than rescue support archetypes (proto 1-3: crude or AI-limited),
// just make the beginner's natural trio damage-capable. Sweep an atk+hp buff on the two
// starter Commons; find what lands the Anchor in 40-60%. greedy draft, AUTO. Sim-only.
...
const STARTERS = new Set(['fizzpop', 'stoneward']); // the two Common starters that drag the Anchor
const COMPS = [
  ...
  { name: 'Glass Cannon', ids: ['cinderpaw', 'shadefang', 'dartwing'] }, // no starters — control
];
const RUNS = parseInt(process.argv[2] || '300', 10);
const BUFFS = [1.0, 1.3, 1.5, 1.7, 2.0]; // atk+hp multiplier on the two starter Commons (1.0 = today)

async function runOne(ids, seed, buff) {
  const W = waves(OUTER), types = ids.map((id) => COMBAT_CREATURES[id].type);
  const runMods = freshMods(), unitMods = ids.map(() => ({}));
  const sm = (id) => (STARTERS.has(id) ? buff : 1.0); // starter buff multiplier
  let hp = ids.map((id) => Math.round(makeUnitDef(id, 'Balanced').hp * rarityMult(id) * sm(id))), alive = ids.map(() => true);
  ...
  for (let wi = 0; wi < W.length; wi++) {
    ...
    ids.forEach((id, idx) => { if (!alive[idx]) return; const b = makeUnitDef(id, 'Balanced'), rm = rarityMult(id);
      const maxHp = Math.round(b.hp * rm * sm(id) * (runMods.hpMult ?? 1));
      units.push({ ...b, hp: Math.min(maxHp, hp[idx]), maxHp, atk: Math.round(b.atk * rm * sm(id)), charge: Math.min(b.maxCharge ?? 6, runMods.chargeStart ?? 0), mods: { ...runMods, ...unitMods[idx] } });
      idxMap.push(idx); });
    ...
    ids.forEach((id, idx) => { if (alive[idx]) { const mh = Math.round(makeUnitDef(id, 'Balanced').hp * rarityMult(id) * sm(id) * (runMods.hpMult ?? 1)); hp[idx] = Math.min(mh, hp[idx] + Math.round(mh * PATCHUP)); } });
  }
...
async function main() {
  ...
  for (const comp of COMPS) { grid[comp.name] = [];
    for (const bf of BUFFS) { let w = 0; for (let i = 0; i < RUNS; i++) if (await runOne(comp.ids, 10000 + i * 7, bf)) w++; grid[comp.name].push(w / RUNS); }
    process.stderr.write(`done ${comp.name}\n`); }
  const pct = (x) => (100 * x).toFixed(1).padStart(6);
  const nStart = (ids) => ids.filter((id) => STARTERS.has(id)).length;
  process.stderr.write(`\n=== PROTOTYPE 4: starter-Common buff (atk+hp; greedy, ${RUNS} runs, AUTO) ===\n`);
  process.stderr.write(`Starter buff ×       ${BUFFS.map((b) => ('×' + b.toFixed(1)).padStart(6)).join('  ')}   #starters\n`);
  process.stderr.write(`${'-'.repeat(20)}${'-'.repeat(BUFFS.length * 8)}\n`);
  for (const comp of COMPS) process.stderr.write(`${comp.name.padEnd(13)} ${grid[comp.name].map(pct).join('  ')}      ${nStart(comp.ids)}\n`);
  process.stderr.write(`\n#starters = how many of the 3 are Fizzpop/Stoneward (buffed). Target: Live Anchor 40-60%.\n`);
  console.log(JSON.stringify({ runs: RUNS, buffs: BUFFS, grid }, null, 2));
}
